...
// Модуль переехал из wxl, а имена, которые он берёт у неё, писались без
// префикса: раньше их находило внешнее пространство wxl. Директива ставит
// поиск на место, не трогая ни строки в самом токенизаторе. В интерфейсе
// модуля её нет -- там типы названы полностью, чтобы ничего не утекло к
// импортирующим.
using namespace wxl;
