...
TEST(format, a_refusal_is_told_in_words_by_its_kind) {
    const auto unreachable =
        std::make_exception_ptr(forum::HttpError(0, wxl::text::repaired(L"нет сети")));
    const auto refused =
        std::make_exception_ptr(forum::HttpError(503, wxl::text::repaired(L"сервер ответил 503")));
