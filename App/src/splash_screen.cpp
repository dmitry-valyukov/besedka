...
SplashScreen::SplashScreen() {
    ...
    // Одна карточка и больше ничего: картинки заставки здесь нет и быть не
    // должно. Она -- задник окна, единственный визуал сцены под прозрачным
    // островом XAML, и второй её экземпляр в дереве был бы тем же битмапом,
    // разложенным дважды: одна копия закрывала бы другую, обе висели бы в
    // памяти, и менять их пришлось бы в двух местах.
    //
    // Обёртки вокруг карточки тоже нет: она сама себе корень экрана, и место
    // на экране (левый нижний угол) держит своим выравниванием в ячейке
    // каркаса.
    root_ = OverlayCard{
        hAlign.left,
        vAlign.bottom,
