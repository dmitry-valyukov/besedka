...
// потока интерфейса. Ответ по сети приходит куда придётся, но дерево JSON
// живёт в пуле, а пул привязан к своему потоку; C++/WinRT возвращает
// корутину туда, откуда её начали, и потому запрос, начатый на потоке
// интерфейса, там же и разбирается. Начатый с чужого потока разберётся на
// чужом -- и это ошибка, которую отладочная сборка пула ловит проверкой.
//
// Кому нужен ответ на другом потоке -- тому :transport и :parse отдельно:
// байты можно возить куда угодно, дерево нельзя.
