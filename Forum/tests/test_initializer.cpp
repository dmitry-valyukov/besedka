...
/// Дерево JSON живёт в пуле STA, а пул строится раз на процесс и на своём
/// потоке. Глобальная обвязка gtest -- ровно то место, где это делается:
/// до первого теста и на всё время до последнего.
